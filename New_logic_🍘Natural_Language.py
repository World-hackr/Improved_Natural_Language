...
class EnvelopePlot:
    ...
    def preview_envelope(self):
        # For preview, we'll just do the old "replace amplitude" approach
...
##############################################################################
# 5) Conditional Blending that sets Undrawn to Zero
##############################################################################
def get_modified_wave_undrawn_zero(ep, alpha=0.3):
    """
    For each sample i:
      - If user-drawn is 0, final[i] = 0 (silence in undrawn region).
      - Else blend original[i] with drawn[i] using alpha.

    We still check sign of original[i] to decide which drawn array to use:
      drawn_val = ep.drawing_pos[i] if original[i] >= 0
                = ep.drawing_neg[i] if original[i] < 0

    Then:
      if drawn_val == 0:
          final[i] = 0
      else:
          final[i] = alpha * original[i] + (1 - alpha) * drawn_val
    """
    original = ep.audio_data
    pos_drawn = ep.drawing_pos
    neg_drawn = ep.drawing_neg

    out = np.copy(original)
    for i in range(len(out)):
        # pick from pos_drawn or neg_drawn based on original sign
        if out[i] >= 0:
            drawn_val = pos_drawn[i]
        else:
            drawn_val = neg_drawn[i]

        if drawn_val == 0.0:
            # user didn't draw => set final to 0
            out[i] = 0.0
        else:
            # blend
            out[i] = alpha * out[i] + (1.0 - alpha) * drawn_val

    return out

##############################################################################
# 6) MAIN (Single-File Processing)
##############################################################################
def process_single_file():
    ...
    # === Construct the Modified Wave with "undrawn => zero" approach ===
    alpha = 0.3  # Tweak as desired
    mod_wave = get_modified_wave_undrawn_zero(ep, alpha=alpha)
